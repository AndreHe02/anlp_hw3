...
class TrainORMData(TrainRMData):
    def __getitem__(self, index):
        question = self.data[index]['question']
        good_solution = random.choice(self.data[index]['good_solution'])
        bad_solution = random.choice(self.data[index]['bad_solution'])
        good_solution = 'Lets think step by step.\n\nStep 1: ' + good_solution
        bad_solution = 'Lets think step by step.\n\nStep 1: ' + bad_solution

        prompt = "Given the question and solution, predict the correctness of the solution."
        good_messages = [{'role': 'user',
        ...
        good_ids, good_lbs = built_example(good_messages, self.tokenizer)
        good_ids, good_lbs = good_ids[:-1], good_lbs[:-1]
        bad_ids, bad_lbs = built_example(bad_messages, self.tokenizer)
        bad_ids, bad_lbs = bad_ids[:-1], bad_lbs[:-1]
        return torch.tensor(good_ids[:4096]), torch.tensor(good_lbs[:4096]), torch.tensor(bad_ids[:4096]), torch.tensor(bad_lbs[:4096])

    def collate_fn(self, batch):
        good_ids, good_lbs, bad_ids, bad_lbs = zip(*batch)
        good_ids = pad_sequence(good_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        good_lbs = pad_sequence(good_lbs, batch_first=True, padding_value=-100)
        bad_ids = pad_sequence(bad_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        bad_lbs = pad_sequence(bad_lbs, batch_first=True, padding_value=-100)
        features = {
            "good_ids": good_ids,
            'good_lbs': good_lbs,
            'bad_ids': bad_ids,
...
class TrainCorrectData(TrainRMData):
    def __getitem__(self, index):
        ...
        if seed < prefix_value:
            ...
            #         f"Wait, I need to rethink my answer... Hmm... there seem to be some issues... lets go back{back_to} and correct {back_step}."] + correct
            # else:
            #     answer = prefix + reject + [
            #         f"Wait, I need to rethink my answer... Hmm... there seem to be some issues... lets go back{back_to} and re-try {back_step}."] + correct
        ...
        messages = [{'role': 'user', 'content': question}, {'role': 'assistant', 'content': answer}]
        ids = self.tokenizer.apply_chat_template(messages, tokenize=True, add_generation_prompt=False)
        q_ids = self.tokenizer.apply_chat_template([{'role': 'user', 'content': question}], tokenize=True, add_generation_prompt=True)
        labels = [-100] * len(q_ids) + ids[len(q_ids):]
...
class TrainPairData(TrainRMData):
    def __getitem__(self, index):
        ...
        return torch.tensor(ids[:2048]), torch.tensor(ids[:2048]), torch.tensor(i_ids[:2048]), torch.tensor(i_ids[:2048])
...
def train_prm():
    ...
    save_path = 'models/Llama-3.1-8B-Instruct-O1'
    ...
    for epoch in range(8):
        ...
        for batch in tk0:
            with accelerator.accumulate(model):
                # loss = model(input_ids=batch['good_ids'], labels=batch['good_lbs']).loss
                # loss += model(input_ids=batch['bad_ids'], labels=batch['bad_lbs']).loss
